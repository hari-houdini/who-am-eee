...
class CardModal extends HTMLElement {
  ...
  constructor() {
    ...
    this.#overlayEl = requireEl<HTMLElement>(
      shadow,
      '.card-modal__overlay',
    );
    this.#frameEl = requireEl<HTMLElement>(
      shadow,
      '.card-modal__frame',
    );
    this.#insetEl = requireEl<HTMLElement>(
      shadow,
      '.card-modal__inset',
    );
    this.#titleEl = requireEl<HTMLElement>(
      shadow,
      '.card-modal__title',
    );
    this.#bodyEl = requireEl<HTMLElement>(
      shadow,
      '.card-modal__body',
    );
    this.#closeBtn = requireEl<HTMLButtonElement>(
      shadow,
      '.card-modal__close',
    );
  }
  ...
  connectedCallback(): void {
    ...
    window.addEventListener('card:open', this.#handleOpen, {
      signal,
    });
  ...
  readonly #handleOpen = (e: Event): void => {
    const { heading, bodyTag, cardId } = (
      e as CustomEvent<CardOpenDetail>
    ).detail;
  ...
  readonly #handleAnimationEnd = (): void => {
    if (!this.#frameEl.classList.contains('card-modal__frame--leave'))
      return;
    this.#frameEl.classList.remove('card-modal__frame--leave');
    while (this.#bodyEl.firstChild) {
      this.#bodyEl.removeChild(this.#bodyEl.firstChild);

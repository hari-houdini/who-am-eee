import { PROJECT_INFO } from "../../shared/const/project.const";
 ...
 * in shadow DOM — no light-DOM children or `content` injection required.
 *
...
class ProjectTabs extends HTMLElement {
	/** Panel `<div>` elements built in shadow DOM. */
	...
	/** The shadow `[role="tablist"]` container. */
	#tabList: HTMLElement | null = null;

	/** Shadow DOM body container for panels. */
	#body: HTMLElement | null = null;

	/** Progress ring host `<div>`. */
	#ringEl: HTMLElement | null = null;

	/** Slot `<div>` that receives the GitHub `<a>` element built in `#loadProject`. */
	...
	constructor() {
		...
		this.#body = shadow.querySelector(".project-tabs__body");
		this.#ringEl = shadow.querySelector(".project-tabs__progress-ring");
		this.#linkSlot = shadow.querySelector(".project-tabs__link-slot");
	}

	/** Wires listeners and loads project data for the current `data-project-id`. */
	...
	#buildTabs(): void {
		...
		this.#panels.forEach((panel, i) => {
			...
			btn.addEventListener("click", () => this.#activate(i), { signal });
